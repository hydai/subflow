...
> **About workflow URLs**: per SPEC §7.5, Subflow's manifest only requests `host_permissions: ["https://www.youtube.com/*"]` — that permission is needed for reading subtitle XML from YouTube and injecting the sidebar into the watch page. Workflow POSTs are sent from the background service worker, which CAN reach any HTTPS URL **without** Subflow declaring that URL in `host_permissions`. The catch: the target endpoint must respond with CORS headers that permit the extension's `chrome-extension://<id>` origin (at minimum `Access-Control-Allow-Origin` and `Access-Control-Allow-Headers`). Endpoints that can't be configured for CORS — such as APIs that strictly reject non-browser origins — are not usable as Subflow workflow targets.
