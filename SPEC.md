...
- Subflow 無後端基礎設施；不收集、不儲存、不轉發任何使用者資料
...
3. 若該優先語言無直接匹配 track，但有任一 track 為 `isTranslatable: true`，挑選最高優先 source（`human > auto`）並在其 `baseUrl` 附加 `&tlang=<優先語言代碼>`，視為 `translation` source
...
| 並行語意 | 多個 `autoRun` 工作流可並行；同一 (videoId, 語言) 的字幕讀取在分頁內為單一 in-flight 請求，重複請求等待同一 promise |
