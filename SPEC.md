...
3. 若該優先語言無直接匹配 track，但有任一 track 為 `isTranslatable: true`，依下列規則衍生翻譯來源：
   ...
   - 在該 track 的 `baseUrl` 附加 `&tlang=<優先語言代碼>`，視為 `translation` source
   - 此 track 的「實際匹配的語言代碼」為附加的 `tlang` 值（即優先語言代碼），而非該 track 原本的 `languageCode`
...
| 顯示內容 | 工作流按鈕列（僅顯示 `autoRun: false` 的工作流；`autoRun: true` 的工作流不出現於按鈕列，僅以結果列表中的條目呈現執行狀態）、最近 5 筆執行結果（成功與失敗都計入）、字幕狀態（已快取／剛讀取／讀取失敗／無可用字幕；錯誤狀態的具體訊息見 §6.6）、手動重新抓取按鈕 |
...
| `{{transcript_with_timestamps}}` | 含時間戳的字幕，每行格式 `[mm:ss] 文字`；影片時長 ≥ 60 分鐘或時長無法取得時改為 `[hh:mm:ss] 文字`，同一支影片內格式統一 |
| `{{title}}` | 影片標題 |
