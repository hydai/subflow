...
- **Per-tab in-memory caches** — for each YouTube tab Subflow keeps two pieces of state in the background service worker's memory: (a) the parsed subtitle for any video viewed in that tab, and (b) the latest extracted player data (video title, channel, length, caption track list and the corresponding `baseUrl`s). Both live only for the lifetime of the tab; closing the tab discards everything via `chrome.tabs.onRemoved`. The service worker itself can be torn down by Chrome at any time when idle, which also clears this state.
- **Per-tab sidebar state** — once the sidebar UI lands (tracked in #12 / #13), it will keep a recent-results list of up to five workflow outputs per SPEC §6.4. This list lives in the sidebar's in-memory state (inside the YouTube page's content-script context), not in the background. It is reset when you switch videos or close the tab.
